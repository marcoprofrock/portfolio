...
- **Baseline wave:** per-glyph `translateY(±wf · pct em)`, slider 0–100 % (= Illustrator `fontSize · pct`). **Defaults to 10 %** on load.
...
- Default load = mixed cuts + colour + Mixed weight + baseline wave 10 %. Tester window background = brandbook **Cloud Dancer** `#F0EEE9` (genuine PANTONE 11-4201 TCX off-white; brandbook prints it as #FFFFFF but pure white would vanish on the light theme) on BOTH dark/light themes.
