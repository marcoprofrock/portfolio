...
- `500ls/index.html` — **Standalone 500l/s case-study page** for the ADCE Student Award submission (`https://www.marco.ad/500ls`). NOT the old portfolio clone (that was removed when the tester went live in `index.html`); this is a fresh, self-contained single-file page. Reuses the main design system + `html.light` toggle; embeds the typeface tester + a 13-spring colour legend; `noindex,nofollow` (robots.txt Disallow `/500ls/`, kept out of sitemap). Loads GA4 directly (NO cookie banner) + fires a custom `adce_case_view` event. Not linked from `index.html`. (See Architecture → "500l/s Case Study".)
...
- Self-contained single-file HTML, **root-relative `/assets`** paths (it lives in a subfolder). Reuses the main design system (Inter preload, CSS vars/spacing, dark default + `html.light` toggle with anti-FOUC head script + localStorage; light-mode logos→black + no shadows).
- Sections top→bottom: minimal header (← back to `/` + theme-toggle pill + "Try the typeface ↓") → hero (colour logo + tagline H1 + Client/Year/Role + award badges) → "The idea" (Problem/Idea/Solution **verbatim from `#p-500ls`**, 3-col desktop) → case film (same lite-yt lazy YouTube `JCJF8uDo0tQ`, "press CC") → key-visuals gallery (`/assets/05_500ls/…`, lightbox) → "The parametric system" (the `.tt` tester + a `.springs` 13-card legend) → credits + awards → footer (hello@marco.ad + ← back + Imprint/Privacy).
